...
test.describe(`Signin Positive Tests with Profile Verification `, { tag: ['@RegressionTesting', '@HappyPath'] }, () => {
    users.forEach((authUser) => {
        ...
        test(`Should sign in as ${authUser.username} and verify profile`, async ({ signinPage, page }, testInfo) => { // Use the signinPage fixture
            ...
            await test.step('Verify Dashboard URL', async () => {
              await signinPage.verifyLoginSuccessful(`/dashboard`); 
            });
        ...
        test(`Should sign in as ${authUser.email} and verify profile`, async ({ signinPage, page }, testInfo) => { // Use the signinPage fixture
            
            await test.step('Alternative Signin Method using JSON Data', async () => {
              await signinPage.login(authUser.email, authUser.password);
            });
